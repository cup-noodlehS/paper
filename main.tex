...
In this study, the detector will use publicly available pre-trained weights (without fine-tuning on the collected videos) to keep the detection component simple and isolate the contribution of tracking-stage stabilization. Specifically, a YOLOv3 model pre-trained on the MS COCO dataset will serve as the detection baseline \parencite{RedmonFarhadi2018,Lin2014COCO}. Only the four vehicle classes within scope (motorcycle, car, bus, truck) are retained; detections outside these classes are discarded during tracking and evaluation.
