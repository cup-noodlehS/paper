...
\section{Rationale}
Road traffic injuries remain a major public-safety concern, causing about 1.19 million deaths globally each year, with the burden falling disproportionately on low- and middle-income countries \parencite{WHO2023Road}. In the Philippines, road crashes continue to impose substantial losses, with the WHO estimating 11,062 road traffic fatalities in 2021 \parencite{WHO2023PH}. Strengthening compliance with traffic laws is therefore a practical lever for improving road safety, but fully manual enforcement is difficult to scale consistently across dense urban road networks \parencite{Olugbade2022}.

As a response, jurisdictions increasingly use camera-based enforcement and video analytics, as reflected locally by the No-Contact Apprehension Policy (NCAP) that relies on recorded visual evidence for apprehension and adjudication \parencite{MMDA2022}. For such systems, the credibility of inferred events depends on stable detection and identity continuity, because many observable violations are defined by where and how a vehicle moves within a scene \parencite{Olugbade2022,Rathore2021}. A vehicle running a red light, entering a restricted lane, or making an illegal U-turn cannot be reliably determined from a single image alone; rather, such events must be inferred from the vehicle's trajectory; its path over time relative to lane boundaries, stop lines, or prohibited zones. This trajectory-based reasoning requires that each vehicle be detected consistently across frames and that its identity be maintained without confusion with other vehicles in the scene.

However, real-world deployments face persistent technical issues that challenge this requirement. Congestion, occlusion, viewpoint changes, and illumination variability degrade detection and tracking in traffic scenes \parencite{Wen2020}. Dataset bias and distribution shift further reduce model reliability when deployment conditions differ from training data \parencite{TorralbaEfros2011,Koh2021}. In practice, such degradation manifests as missed detections when vehicles are partially hidden, spurious detections triggered by shadows or reflections, and inconsistent class labels that fluctuate between frames. These per-frame inconsistencies propagate into tracking instability and identity switches in multi-object tracking (MOT), where a single physical vehicle may be assigned multiple identities or confused with nearby vehicles \parencite{Wen2020,Zhang2022}. When trajectories become fragmented or corrupted, geometric rules that rely on continuous paths---such as detecting whether a vehicle crossed a boundary or stopped in a prohibited zone---can produce erroneous or missed violation inferences.
...
\begin{itemize}
  ...
  \item \textbf{Tracking stability:} Multiple Object Tracking Accuracy/Precision (MOTA/MOTP) \parencite{BernardinStiefelhagen2008} and measures, particularly identification F1 score (IDF1) and ID switches \parencite{Ristani2016}.
...
Real-world CCTV deployments must contend with occlusion, viewpoint changes, and illumination variability that degrade camera-based detection and tracking \parencite{Wen2020}. Rain can further reduce perception reliability \parencite{Brophy2023}, and dataset bias and distribution shift can degrade model behavior when target scenes exhibit characteristics not represented in training corpora \parencite{TorralbaEfros2011,Koh2021}.

Deployments that process CCTV footage must also comply with Philippine data-protection requirements, including the Data Privacy Act of 2012 and the National Privacy Commission's policy guidance on CCTV systems \parencite{RA10173,NPCCircular2024CCTV}.
...
However, dataset bias and distribution shift can cause inconsistent detections and class outputs when deployment conditions differ from training data, and these inconsistencies can propagate into tracking instability \parencite{TorralbaEfros2011,Koh2021}. Accordingly, detector operating points should be explicitly evaluated, and detection metrics should be reported alongside tracking and event-level outcomes to characterize how upstream variability affects downstream inference.
...
Figure~\ref{fig:pipeline-architecture} illustrates the overall architecture and data flow of the proposed pipeline, highlighting the role of fuzzy logic enhancement in the tracking stage and the calibration dependencies for speed estimation and violation inference.

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.95\textwidth]{images/pipeline_architecture_2.png}
  \caption{ATVD pipeline architecture showing the end-to-end processing flow: (1) CCTV video input, (2) YOLO-based vehicle detection, (3) multi-object tracking with fuzzy association and class stabilization, (4) calibration-dependent speed estimation, and (5) rule-based violation inference producing evidence bundles. Dashed lines indicate calibration dependencies.}
